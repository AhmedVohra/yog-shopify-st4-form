...
email.js                 Microsoft Graph client — emails the signed PDF copy and the ST-4 form-link email
...
5. A **Business Central bridge** — a two-way integration with a separate Azure Function that owns the customer-application records in Azure SQL / BC (see "Azure Function round-trip" below)
...
Two optional per-template fields support multi-form intake flows (see "Azure Function round-trip" below):
- `nextTemplateId` — if set, a successful customer-form submission redirects into that template's form instead of `/pages/pending-approval`, carrying `email`/`applicationId`/`name` forward. Unset on a template (the common case, e.g. the ST-4 template) preserves today's single-form behavior exactly.
- `formType` — a short tag (e.g. `st4`, `page3`) used only to build the Azure Function notify `type={formType}notify` and the corresponding `{formType}PdfUrl`/`{formType}FormAttached` field names; unset resolves to `'st4'` via `resolveFormType()` in `shopify-server.js`, so pre-existing templates need no migration. Both fields are editable from the Template Designer toolbar ("Next template ID" / "Form type" inputs) — there's no other way to set them.

### PDF handling — client-side, not server-side

The server never rasterizes or edits PDFs at runtime. All rendering (`pdf.js`) and field-writing (`pdf-lib`) for the actual signing flow happens in the browser, loaded from CDN in `public/template-designer.html` and `public/customer-form.html` (not npm dependencies — check those `<script src>` tags if bumping versions). The server's only PDF-related job is accepting a final signed PDF as base64 (`handleSaveSignedPdf` in `shopify-server.js`) and uploading the bytes to a SharePoint document library via `uploadPdfToSharePoint` in `sharepoint.js` (Microsoft Graph client-credentials flow: token from `login.microsoftonline.com`, lazy site/drive resolution, then a simple `PUT …:/content` — fine because signed PDFs are well under Graph's 4 MB simple-upload limit). Stored filenames get a timestamp prefix because Graph PUT silently overwrites same-name files. Separately, `pdf-lib` also exists as a **devDependency** in `package.json` (not the CDN copy above) purely for one-off `scripts/` PDF manipulation like `extract_page3.js` — see "One-off scripts" below; it plays no role in the running server.
...
**Per-lead subfolders.** `uploadPdfToSharePoint(filename, pdfBuffer, folderSegment)` takes an optional third argument — when a submission carries both `applicationId` and `name` in its `save-signed-pdf` body, `handleSaveSignedPdf` builds `folderSegment` via `buildLeadFolderName(name, applicationId)` (e.g. `"Jane_Doe - e07a1234"`, sanitized name + last-8-alphanumeric-chars-of-applicationId suffix for uniqueness between same-named customers) so a lead's multiple form PDFs (see "Azure Function round-trip" below) land together instead of flat in the root. Omitting `folderSegment` keeps the original flat-root behavior — e.g. standalone/manual test submissions with no `applicationId`. Graph's path-based `PUT .../root:/{path}:/content` does **not** auto-create missing intermediate folders (confirmed against Microsoft's own upload-flow docs), so `ensureLeadFolder()` does an explicit GET-then-POST-children-with-conflictBehavior-fail before the upload whenever a folder segment is given — a concurrent 409 (the lead's second PDF arriving close behind the first) is treated as success, not an error.
...
`submitForm()` converts the flattened PDF bytes to base64 via a chunked `bytesToBase64()` helper, **not** `btoa(String.fromCharCode(...bytes))` — that spread form passes every byte as a separate function argument, which throws `RangeError: Maximum call stack size exceeded` once a PDF (plus any embedded signature PNG) crosses the browser's argument-count ceiling (~65k). This is easy to miss locally against small test PDFs and only surfaces on larger real-world forms (first hit on the page-3 "Credit Terms" PDF, not the smaller ST-4 one) — don't reintroduce the spread form here even as a "simplification."

`customer-form.html` reads three optional query params at load — `?email=` (`customerEmail`), `?applicationId=`, and `?name=` (`customerName`) — and, if present, includes them (plus `templateId`, read from the loaded template object) in the `save-signed-pdf` POST body. `email` is how a copy of the signed PDF gets emailed to the customer server-side (see `email.js` above); `applicationId` is how the submission gets linked back to the customer's Business Central application record (see "Azure Function round-trip" below); `name` drives the SharePoint per-lead folder name (see `sharepoint.js` above) and is carried forward through any chained redirect. All three are meant to be carried in the link the customer clicked (generated by `/api/send-st4-link`, the storefront's redirect, or the Designer's Share button), not typed in on the form itself.

After a successful submit, if the loaded template has `nextTemplateId` set, the form redirects into that next template's form instead (`/form?id={nextTemplateId}&email=…&applicationId=…&name=…`, in whichever of proxy/direct mode the current page is in) — this is the multi-form chaining mechanism (see "Azure Function round-trip" below). Otherwise, in App Proxy mode (`apiBase` starting with `/apps/`) the form redirects back to `/pages/pending-approval` on the storefront after a short delay, unchanged from before; standalone/direct mode never redirects when there's no `nextTemplateId`, since there's no store page to land on there.
...
**A lead now signs two chained PDFs, not one.** A "Customer Application — Page 3" template (`formType: "page3"`, `nextTemplateId` set to the ST-4 template's id) was inserted between the storefront apply-account step and the ST-4 form: apply-account → page-3 form → ST-4 form → `/pages/pending-approval`. The chain mechanism (`nextTemplateId` on the template, `customer-form.html`'s post-submit redirect) is generic — see "Template storage" and the customer-form query-param paragraph above — the Function-side wiring below is what makes each link in the chain notify BC/SQL correctly.

The round-trip:

1. Customer submits an application on the storefront (`form.liquid`, in the function's repo) → the Azure Function saves it and generates an `applicationId` (`APP-…`).
2. The function calls **this** server's `POST /api/send-st4-link` (`{email, customerName, applicationId, templateId}`) → we email the customer a personalised form link via `sendST4FormLinkEmail` in `email.js`, now including `&name=` (URL-encoded `customerName`) alongside `email`/`applicationId` so it survives into any chained redirect too. The link prefers the store-proxied URL (`https://{SHOPIFY_STORE}/apps/pdf-signer/form?id=…&email=…&applicationId=…&name=…`) and falls back to `APP_URL` direct. The function knows our URL via its `SHOPIFY_SERVER_URL` app setting and the template via `ST4_TEMPLATE_ID` — despite the name, this can point at either form in the chain depending on which one the function wants to email a link for; if the customer form's URL shape or a template ID changes, that function-app setting has to change too. The storefront's own immediate post-signup redirect (inline JS in the theme editor, not in any repo) is the other, non-email path onto the chain — it should point at the **page-3** template's id, not ST-4's, now that page-3 is first in the chain.
3. Customer signs and submits (either link in the chain) → `handleSaveSignedPdf` uploads to SharePoint (grouped into a per-lead folder — see `sharepoint.js` above), resolves `formType` from the submitted `templateId` via `resolveFormType()` (defaults to `'st4'` when `templateId` is absent or the template has no `formType` set), then — if `applicationId` was in the body — notifies the function with `POST {AZURE_FUNCTION_URL}&type={formType}notify` and body `{applicationId, [formType+'PdfUrl']: fileUrl}` (e.g. `type=st4notify`/`st4PdfUrl`, or `type=page3notify`/`page3PdfUrl`) so it updates the matching SQL columns and, for `st4notify` only, BC (BC sync for `page3notify` is deliberately deferred on the function side pending a product decision). Best-effort: a failed notify is logged and reported as `bcNotified: false` but never fails the submission (the PDF is already safely stored).
4. Because a customer can reach any given form in the chain twice (the immediate redirect and the emailed fallback link both eventually point at the same forms), `customer-form.html`'s `init()` guards against re-showing an already-signed form: before rendering the PDF, it calls **this** server's `GET /api/st4-status/:applicationId?templateId=…` (`/proxy/api/st4-status/:applicationId` when proxied), which resolves `formType` from the given `templateId` the same way `handleSaveSignedPdf` does, then does a plain `GET {AZURE_FUNCTION_URL}&applicationId=…` against the same Function — unlike step 3's notify, plain GET needs no `x-api-key`, just the `?code=` already in `AZURE_FUNCTION_URL` — and reports `submitted: true` once `{formType}FormAttached` is set on that row. If already submitted, the form shows a terminal "already submitted" message instead of letting the customer fill and resubmit. This check is deliberately best-effort/fail-open (8s server-side `AbortController` timeout in `handleST4Status`, 10s client-side in `checkAlreadySubmitted()`) — the Function's Azure SQL backend auto-pauses when idle and can take up to ~60s to resume, and a slow/unreachable check must never block a legitimate first-time signer. Omitting `?templateId=` (or passing an unrecognized one) falls back to checking `st4FormAttached`, identical to pre-chaining behavior.

**Two separate keys, both required, on the outbound call** (this has broken in production before — see `.env.example`):
- `AZURE_FUNCTION_URL` must be the full trigger URL **including** `?code=<function key>` — the function's authLevel is `function`, so Azure rejects the request before the handler runs without it. Because the URL already carries a query string, the server appends `type={formType}notify` with `&`/`?` chosen by `azureUrl.includes('?')` — don't "simplify" that back to a bare `?type=…`, which produces a double-`?` URL and a platform 401.
- `AZURE_FUNCTION_API_KEY` is sent as the `x-api-key` header and must match one of the comma-separated `API_KEYS` app-setting values on the function app — that's the function's own app-level gate on `st4notify`/`page3notify` (and its other protected types), layered on top of the platform key. Both notify types share this one key; no separate key is needed per form type.

`/api/send-st4-link` is **direct-route only** (no `/proxy/` twin) — it's server-to-server from the Azure Function, never called from the storefront. The Designer's Share and Preview buttons also prompt for an optional `applicationId` to embed in generated form links, for manually sending a link outside the automated email flow.

**Function-side status (verified live):** `type=page3notify` is deployed on the Function (`handlePage3Notify` in its `index.js`, mirroring `handleST4Notify`) and the five `page3*` columns (`page3FormAttached`, `page3PdfUrl`, `page3SignedAt`, `page3Status`, `page3Fields`) exist on `dbo.Applications` — confirmed with a non-destructive no-op `page3notify` call against a nonexistent `applicationId`, which returned `200` rather than a SQL "invalid column" error. `handleGet` on the function side does `SELECT * FROM dbo.Applications`, so the new columns flow through the existing status-check response automatically with no function-side code change needed for that part. BC sync for page-3 is an explicit `TODO` in `handlePage3Notify` — not wired up, by design, until there's a product need for it.
...
Older deploy configs are still checked in and kept in sync manually: `render.yaml` (Render), `fly.toml` + `Dockerfile` (Fly.io), and `Procfile` (Heroku-style) — all just run `npm start`, but none of them have been exercised since the move to Azure App Service; treat them as unverified. `.env.example` is the canonical list of required env vars (Shopify proxy secret + `MS_*`/`SHAREPOINT_*` Graph settings for SharePoint, `MAIL_*` for email, `AZURE_FUNCTION_*` for the BC notify call — see "Two Graph credential sets" and "Azure Function round-trip" above).

### One-off scripts (`scripts/`)

`scripts/make_pdf.js`, `scripts/query_page.js`, `scripts/update_page.js`, `scripts/update_page2.js`, `scripts/extract_page3.js`, and `scripts/tunnel.bat` are throwaway dev scripts for manipulating the dev store directly (generating a demo PDF into `data/demo.pdf`, reading/writing a Shopify Page's `body_html` to embed the form iframe) — they are not imported by the server and aren't part of the running app. They load `SHOPIFY_STORE`/`SHOPIFY_ADMIN_API_TOKEN` from the root `.env` via `dotenv` — never hardcode a token back into these files, since (unlike `.env`) they're tracked by git. `update_page.js` and `update_page2.js` are two historical variants of the same one-off task (different tunnel URLs); nothing currently depends on either running again.

`extract_page3.js` (no `.env`/network dependency, pure filesystem) pulls page 3 out of `data/Customer Application 1.pdf` into its own single-page `data/customer-application-page3.pdf`, using the `pdf-lib` devDependency (see "PDF handling" above). It exists because `customer-form.html` always renders page 1 of whatever PDF a template stores (`pdfDoc.getPage(1)`, hardcoded) — building a template from page 3 of a multi-page source requires extracting that page into its own file first; the Template Designer itself has no page-selection UI for templates. Re-run it only if `data/customer-application-page3.pdf` needs regenerating (e.g. the source PDF changes); it's a one-shot input to a manual Designer step, not something the running app calls.
