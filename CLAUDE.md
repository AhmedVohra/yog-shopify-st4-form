...
`customer-form.html` reads two optional query params at load — `?email=` (`customerEmail`) and `?applicationId=` — and, if present, includes them in the `save-signed-pdf` POST body. `email` is how a copy of the signed PDF gets emailed to the customer server-side (see `email.js` above); `applicationId` is how the submission gets linked back to the customer's Business Central application record (see "Azure Function round-trip" below). Both are meant to be carried in the link the customer clicked (generated by `/api/send-st4-link` or the Designer's Share button), not typed in on the ST-4 form itself. After a successful submit in App Proxy mode (`apiBase` starting with `/apps/`), the form redirects back to `/pages/pending-approval` on the storefront after a short delay; standalone/direct mode never redirects, since there's no store page to land on there.

### Azure Function round-trip — Business Central integration

The customer-application lifecycle spans this app and a **separate Azure Function** (`CustomerApplication` on Function App `yelloglow-forms`, resource group `yell-o-glow-rg` — its source lives in its own repo at `C:\Users\ahmed\OneDrive\Documents\Yog shopify customer application form`, which has its own CLAUDE.md; don't edit it from here). The function owns the application records (Azure SQL `dbo.Applications`) and the NTLM calls into Business Central. The round-trip:
...
2. The function calls **this** server's `POST /api/send-st4-link` (`{email, customerName, applicationId, templateId}`) → we email the customer a personalised form link via `sendST4FormLinkEmail` in `email.js`. The link prefers the store-proxied URL (`https://{SHOPIFY_STORE}/apps/pdf-signer/form?id=…&email=…&applicationId=…`) and falls back to `APP_URL` direct. The function knows our URL via its `SHOPIFY_SERVER_URL` app setting and the template via `ST4_TEMPLATE_ID` — if the customer form's URL shape or a template ID changes, those function-app settings have to change too.
3. Customer signs and submits → `handleSaveSignedPdf` uploads to SharePoint, then (if `applicationId` was in the body) notifies the function with `POST {AZURE_FUNCTION_URL}&type=st4notify` and body `{applicationId, st4PdfUrl}` so it updates the SQL record and BC. Best-effort: a failed notify is logged and reported as `bcNotified: false` but never fails the submission (the PDF is already safely stored).
4. Because step 2's link and the storefront's own immediate post-signup redirect both point the customer at the same ST-4 form for the same `applicationId`, a customer who finishes via one path and then opens the other would otherwise see a blank fillable form again. `customer-form.html`'s `init()` guards against this: before rendering the PDF, it calls **this** server's `GET /api/st4-status/:applicationId` (`/proxy/api/st4-status/:applicationId` when proxied), which does a plain `GET {AZURE_FUNCTION_URL}&applicationId=…` against the same Function — unlike step 3's `type=st4notify`, plain GET needs no `x-api-key`, just the `?code=` already in `AZURE_FUNCTION_URL` — and reports `submitted: true` once `st4FormAttached` is set on that row. If already submitted, the form shows a terminal "already submitted" message instead of letting the customer fill and resubmit. This check is deliberately best-effort/fail-open (8s server-side `AbortController` timeout in `handleST4Status`, 10s client-side in `checkAlreadySubmitted()`) — the Function's Azure SQL backend auto-pauses when idle and can take up to ~60s to resume, and a slow/unreachable check must never block a legitimate first-time signer.

**Two separate keys, both required, on the outbound call** (this has broken in production before — see `.env.example`):
- `AZURE_FUNCTION_URL` must be the full trigger URL **including** `?code=<function key>` — the function's authLevel is `function`, so Azure rejects the request before the handler runs without it. Because the URL already carries a query string, the server appends `type=st4notify` with `&`/`?` chosen by `azureUrl.includes('?')` — don't "simplify" that back to a bare `?type=…`, which produces a double-`?` URL and a platform 401.
- `AZURE_FUNCTION_API_KEY` is sent as the `x-api-key` header and must match one of the comma-separated `API_KEYS` app-setting values on the function app — that's the function's own app-level gate on `type=st4notify` (and its other protected types), layered on top of the platform key.
